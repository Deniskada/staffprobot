#!/usr/bin/env python3
"""
API сервер StaffProBot с поддержкой базы данных
"""
import json
import time
import sys
...
class DatabaseAPIHandler(BaseHTTPRequestHandler):
    """HTTP обработчик для API с базой данных."""
        ...
        try:
            ...
            if path == "/":
                ...
                self._send_json_response({
                    ...
                    "status": "running",
                    "database": "connected" if hasattr(db_manager, '_initialized') and db_manager._initialized else "disconnected",
                    "docs": "disabled"
                })
                ...
                self._send_json_response({
                    ...
                    "timestamp": time.time(),
                    "version": settings.version,
                    "database": "connected" if DB_AVAILABLE and hasattr(db_manager, '_initialized') and db_manager._initialized else "disconnected"
                })
                ...
                if DB_AVAILABLE:
                    ...
                    self._send_error_response("not_found", "Database not available", 503)
...
def main():
    ...
    try:
        ...
        # Создаем сервер
        server = HTTPServer((settings.api_host, settings.api_port), DatabaseAPIHandler)
