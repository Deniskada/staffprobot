...
- [ ] **6.1. Создать систему уведомлений о заявках**
  - [ ] 6.1.1. Создать общую модель уведомлений и миграцию (таблица `notifications`)
  - [ ] 6.1.2. Реализовать `NotificationService` в `shared/services` (web + Telegram)
  - [ ] 6.1.3. Создать REST API `/api/notifications` и UI-виджет (бейдж + список)
  - [ ] 6.1.4. Интегрировать сервис в события заявок (создание, одобрение, отказ)
- [ ] **6.2. Создать систему уведомлений о собеседованиях**
  - [ ] 6.2.1. Добавить типы уведомлений для собеседований (назначение, перенос, отмена)
  - [ ] 6.2.2. Реализовать напоминания (планировщик + Telegram)
