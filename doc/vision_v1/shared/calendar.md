...
- `shared/services/calendar_filter_service.py` — CalendarFilterService с кэшированием методов _get_timeslots() и _get_shifts()
  ...
  - **Логика `has_free_track` для текущего дня (обновлено 09.01.2026):**
    - Для тайм-слотов текущего дня, которые уже начались (`slot_start_local <= now_local`) и еще не закончились (`slot_end_local > now_local`), если есть активные смены, то `has_free_track = False` (нельзя добавить новую смену в уже начавшийся тайм-слот)
    - Активные смены проверяются как в `direct_shifts` (с `time_slot_id`), так и в `fallback_candidates` (без `time_slot_id`, но по объекту)
    - Активные смены учитываются при вычислении `current_employees` для корректного отображения занятости
...
- `static/js/shared/universal_calendar.js` — универсальный менеджер календаря с ленивой подгрузкой месяцев (scroll → `loadMonthRange()`), повторным `renderCalendarGrid` и сохранением позиции скролла
  ...
  - **Логика скрытия для текущего дня (обновлено 09.01.2026):**
    - Для текущего дня тайм-слот скрывается если `has_free_track=false`, даже если `current_employees=0` (тайм-слот уже начался и занят активными сменами)
    - Условие: `!hasFreeTrack && (isToday || currentEmployees > 0)` - для текущего дня достаточно `has_free_track=false`
