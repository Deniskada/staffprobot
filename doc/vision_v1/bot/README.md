...
- `schedule_date_YYYYMMDD` — выбор даты планирования (14 кнопок вперёд, по две в строке)
- `schedule_interval_<slotId>_<HHMM>_<HHMM>[_<position>]` — выбор свободного интервала внутри тайм-слота
- `schedule_shift`
...
### Планирование смен (обновлено 13.11.2025)
- После выбора объекта бот показывает клавиатуру с датами на ближайшие 14 дней (`schedule_date_YYYYMMDD`), выводимыми по две в строке (формат `12.11 (Ср)`).
- При выборе даты строится список свободных интервалов из shared-сервиса: каждая кнопка `schedule_interval_*` содержит тайм-слот, время начала/окончания и позицию (`max_employees`).
- Пользователь всегда бронирует весь свободный интервал; частичное смещение внутри окна недоступно.
- Если для даты нет доступных интервалов, бот сообщает об отсутствии свободного времени и предлагает выбрать другую дату.
