...
### Для владельца
- **GET** `/owner/payroll` — интерфейс начислений с двумя вкладками
  - **Вкладка "Сводка по сотрудникам"** (дефолтная):
    - Агрегирует количество начислений, суммарную выплату, последний период и ставку (название договора + ставка) по каждому сотруднику
    - Столбцы: Сотрудник, Начислений, Ставка, Сумма, Последнее начисление, Действия
    - Сортировка по показателям (клик по заголовку столбца): ФИО, количество, ставка, сумма, период
    - Фильтры: Объект, Сотрудник (включая уволенных), Дата выплаты (вместо периода с/по), Показать уволенных (снимаемая галочка)
  ...
  - **Вкладка "Начисления"**:
    ...
    - Столбцы: ID, Сотрудник, Период, Дата выплаты, За смены (было "Начислено"), Удержано, Доплаты, К выплате, Начислено (сумма добавленных выплат), Выплачено (сумма подтвержденных выплат), Действия
    - Убраны столбцы: "Часов", "Ставка"
    - Фильтры: Объект, Сотрудник (включая уволенных, загружается динамически при выборе объекта), Дата выплаты (вместо периода с/по), Показать уволенных (снимаемая галочка)
    - Фильтры применяются автоматически при изменении
...
- **GET** `/owner/payroll/report` — HTML-отчёт по начислениям/выплатам за период, вкладки «Отчёт по начислениям» и «Отчёт по выплатам», кнопки печати и экспорта.
- **GET** `/owner/payroll/report/export` — Excel-версия отчёта (2 листа: «Отчет по начислениям», «Отчет по выплатам»).
- **GET** `/owner/payroll/{entry_id}` — детализация начисления
  - Сводная таблица начислений сотрудника отображается сверху; текущая запись подсвечивается. Параметры `origin=entries|summary` и `selected_entry_id` передаются из списка для позиционирования активной строки (кнопка «Детали» во вкладке «Начисления» передаёт конкретный ID, «Сводка по сотрудникам» подсвечивает последнее начисление).
  - Раздел «Выплаты» показывается перед протоколом изменений; удержания/доплаты и запись выплат блокируются, если уже есть подтверждённые выплаты.
...
- **GET** `/manager/payroll` — сводка начислений (вкладки «Сводка по сотрудникам» / «Начисления») с фильтрами по периоду, объекту и переключателем «Показать уволенных сотрудников». Доступ проверяется через `ManagerPermissionService`; включаются сотрудники по активным и архивным договорам доступных объектов.
- **GET** `/manager/payroll/{entry_id}` — детализация начисления. Загружается по `entry_id`, но верхняя таблица содержит все начисления сотрудника. Доступ к сотруднику проверяется с `include_inactive=True`. Управляющий не подтверждает выплаты, но видит их состояние и может редактировать корректировки до появления выплат.
