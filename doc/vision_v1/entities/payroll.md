...
- **GET** `/owner/payroll` — интерфейс начислений с двумя вкладками
  - «Сводка по сотрудникам» (дефолтная): агрегирует количество начислений, суммарную выплату и последний период по каждому сотруднику, поддерживает сортировку по показателям, фильтры по периоду/объекту и переключатель «Показать уволенных» (отображает сотрудников с завершёнными договорами, помечает бейджем).
  - «Начисления»: плоский список записей (сортировка и пагинация как на `/owner/shifts/`). Фильтр «Сотрудник» подбирает сотрудников по пересечению периода договора (`start_date` vs `COALESCE(end_date, termination_date)`).
