...
- **GET** `/owner/payroll/{entry_id}` — детализация начисления
  - Верхняя таблица содержит все начисления сотрудника (сортировка по периоду/ID/нетто-сумме). Выбор строки обновляет подробный блок.
  - Раздел «Выплаты» отображается перед протоколом изменений; действия по удержаниям/доплатам недоступны при наличии подтверждённых выплат.
