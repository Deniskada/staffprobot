...
Система автоматически протоколирует все изменения критичных полей договора для ретроспективного просмотра и использования исторических данных при расчете начислений.

### Отслеживаемые поля

Система протоколирует изменения следующих полей:
...
### Модель ContractHistory

**Таблица:** `contract_history`
...
- `change_type` (string) - тип изменения: 'created', 'updated', 'status_changed'
- `field_name` (string) - название измененного поля
- `old_value` (JSONB) - старое значение
- `new_value` (JSONB) - новое значение
- `change_reason` (text, nullable) - причина изменения
- `effective_from` (timestamp, nullable) - дата начала действия (для будущих изменений)
- `change_metadata` (JSONB, nullable) - дополнительные данные
...
- При создании договора записывается начальное состояние всех отслеживаемых полей (`change_type='created'`)
- При обновлении договора записываются только измененные поля (`change_type='updated'` или `'status_changed'`)
- Интегрировано в методы:
  - `ContractService.create_contract()`
  - `ContractService.update_contract()`
  - `ContractService.update_contract_for_manager()`
  - `ContractService.update_contract_by_telegram_id()`

**Использование исторических данных:**
- При открытии смены используется `get_contract_snapshot()` для определения действующей ставки на дату смены
- Это гарантирует, что ставка соответствует состоянию договора на момент открытия смены, даже если договор был изменен позже

### API для истории

**Роуты:**
- `GET /owner/contracts/{contract_id}/history` - полная история изменений (JSON API)
  - Параметры: `field_name` (опционально), `limit`, `offset`
- `GET /owner/contracts/{contract_id}/snapshot?date=YYYY-MM-DD` - снимок договора на дату (JSON API)

### UI для истории

**Вкладка "История" на странице просмотра договора:**
- `/owner/employees/contract/{contract_id}` - вкладка "История изменений"
- Отображает таблицу всех изменений с фильтрацией по полям
- Показывает: дату/время, поле, старое/новое значение, кто изменил, тип изменения

### Миграция данных

При внедрении системы создается начальная история для всех существующих договоров (снимок текущего состояния с `change_type='created'`).
