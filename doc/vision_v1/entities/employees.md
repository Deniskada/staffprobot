...
## Веб-роуты и API
- [GET] `/` — (apps/web/routes/owner.py)  — вход владельца (/owner/) → дашборд владельца | роль: owner
- [GET] `/` — (apps/web/routes/manager.py) — редирект управляющего (/manager/ → /manager/dashboard) | роль: owner
...
- [GET] `/dashboard` — (apps/web/routes/owner.py) — служебный редирект на `/owner/`
- [GET] `/dashboard` — (apps/web/routes/manager.py) — дашборд управляющего
