...
StaffProBot — платформа для управления сменами, персоналом и выплатами. Владельцы и управляющие ведут объекты и расписания; сотрудники работают по сменам; система считает зарплату, отправляет уведомления и поддерживает договоры и шаблоны документов. Доступ — через ботов в Telegram и MAX или веб-приложение (FastAPI); роли: владелец, управляющий, сотрудник, соискатель, суперадмин. Реализованы геолокация (PostGIS), календарное планирование с тайм-слотами, шаблоны и версии договоров, расчёт зарплаты с корректировками и биллингом, отзывы и рейтинги, уведомления со ссылками авто-логина в веб.
...
- **Уведомления:** Telegram, MAX и веб; типы (смена, договор, оффер, напоминания и др.); в каждое сообщение подставляется URL с авто-логином (NotificationActionService.get_action_url); владельцы настраивают каналы доставки.
- **Боты (Telegram + MAX):** единая бизнес-логика через `UnifiedBotRouter` — смены, объекты, планирование, задачи, отчёты, геолокация; адаптеры `TgAdapter`/`MaxAdapter` нормализуют входящие апдейты, `TgMessenger`/`MaxMessenger` — исходящие; привязка аккаунтов через одноразовые коды в ЛК.
- **Мульти-мессенджер архитектура:** `NormalizedUpdate` DTO, `messenger_accounts` (один пользователь — несколько мессенджеров), `notification_targets` (группы отчётов TG+MAX на уровне объекта/организации), feature-флаг `MAX_FEATURES_ENABLED` с откатом.
...
| **MAX platform-api.max.ru** | Webhook MAX-бота, сообщения, привязка аккаунтов |
| **MinIO/S3** | Хранение документов и медиа (опционально мессенджеры) |
