...
**Статус:** Завершено (11.01.2026)
- **Проблемы:**
  1. Уведомление "объект открылся с опозданием" приходило с неправильным временем опоздания - смещение на 30 минут (79 минут фактически, показывалось 49)
  2. Уведомление "объект открылся" приходило с неправильно рассчитанным временем своевременности открытия - то же смещение на 30 минут
  3. При своевременном закрытии объекта вместо сотрудника показывался прочерк
  4. Уведомление "Нет смен на объекте" сегодня не приходило совсем - логика была слишком усложнена, хотя на дашборде статус отображался корректно
- **Причины:**
  1. В Celery-задаче использовался timezone объекта (`obj_tz.localize()` и `timezone_helper.utc_to_local()` с `timezone_str=obj_timezone`), а на дашборде использовался default_timezone (`timezone_helper.local_tz.localize()` и `timezone_helper.utc_to_local()` без timezone_str)
  2. В Celery-задаче использовалась логика с `first_shift` (первая смена по времени), а на дашборде использовалась логика с `candidates_after` (первая смена после `expected_open`)
  3. При своевременном закрытии использовалась та же логика, что и при раннем закрытии, где `work_employee` не перезаписывался, если уже был установлен
  4. Логика уведомления "Нет смен на объекте" была слишком усложнена (использовался timezone объекта, сложные проверки), хотя на дашборде использовался простой подход с default_timezone
- **Решение:**
  1. **Синхронизация логики открытия объекта:**
     - В Celery-задаче теперь используется `timezone_helper.local_tz.localize()` для `expected_open` (как на дашборде)
     - Используется `timezone_helper.utc_to_local()` без `timezone_str` для всех расчетов (как на дашборде)
     - Реализована та же логика `candidates_after`: ищется первая смена, которая началась после `expected_open`
     - Если нет `candidates_after`, но есть смены, берется первая смена дня (как на дашборде)
  2. **Исправление отображения сотрудника при своевременном закрытии:**
     - При своевременном закрытии `work_employee` всегда устанавливается `closer_employee`, если он есть (убрана проверка `elif not work_employee`)
  3. **Упрощение логики уведомления "Нет смен на объекте":**
     - Используется default_timezone (`timezone_helper.local_tz.localize()` и `timezone_helper.utc_to_local()` без timezone_str)
     - Добавлена проверка существования уведомления по дате, чтобы не создавать дубликаты
     - Логика полностью соответствует дашборду
- **Результаты:**
  - ✅ Уведомления об открытии объекта приходят с правильным временем (без смещения на 30 минут)
  - ✅ Расчет опоздания в уведомлениях полностью соответствует дашборду
  - ✅ При своевременном закрытии корректно отображается сотрудник
  - ✅ Уведомление "Нет смен на объекте" приходит корректно
- **Файлы:**
  - `core/celery/tasks/reminder_tasks.py` — синхронизирована логика открытия с дашбордом (использование default_timezone и candidates_after), упрощена логика "Нет смен на объекте" (~100 строк изменений)
  - `apps/web/routes/owner.py` — исправлена логика отображения сотрудника при своевременном закрытии (~5 строк изменений)
- **Правила документации:**
  - При изменении логики расчета времени открытия/закрытия объекта ВСЕГДА синхронизировать логику между дашбордом и Celery-задачами
  - Для открытия объекта использовать default_timezone (`timezone_helper.local_tz.localize()` и `timezone_helper.utc_to_local()` без timezone_str) и логику `candidates_after`
  - Для закрытия объекта использовать timezone объекта (как на дашборде)
  - При своевременном закрытии всегда устанавливать `work_employee` = `closer_employee`, если он есть
  - Логика уведомлений должна полностью соответствовать логике дашборда - не усложнять без необходимости

### Исправление отображения сотрудника на дашборде и расчета раннего закрытия в уведомлениях ✅

**Статус:** Завершено (10.01.2026)
- **Проблемы:**
  1. На дашборде владельца при своевременном открытии/закрытии объекта сотрудник не отображался (прочерки)
  2. Уведомление "Объект закрылся раньше" приходило при своевременном закрытии (например, плановое 21:00, фактическое 21:00, но показывалось "Раньше на: 29 мин")
  3. Статус "Нет смен на объекте" устанавливался вне рабочего времени объекта (например, ночью, когда объект уже закрыт)
- **Причины:**
  1. Если смена начиналась до `expected_open` (времени открытия объекта), она не попадала в `candidates_after`, и `opener_shift` становился `None`. При закрытии объекта `work_employee` перезаписывался, даже если уже был установлен при открытии.
  2. В Celery-задаче `check_object_openings` использовалась другая логика расчета `early_minutes` по сравнению с дашбордом: использовался `today_obj_local` (дата в timezone объекта) вместо `today_local` (дата в default_timezone), и условия проверки закрытия отличались.
  3. При отсутствии смен проверялось только, что `now_local >= expected_open`, но не проверялось, что время находится в рабочем времени (`now_local <= expected_close`). Также при установке статуса после закрытия не проверялось, что текущее время не превышает разумный предел (не более 2 часов после рабочего времени).
- **Решение:**
  1. **Исправление логики `opener_shift`:**
     - Если нет `candidates_after` (смен, которые начались после `expected_open`), но есть смены на сегодня, берется первая смена дня по времени начала как открывающая объект
     - При закрытии объекта `work_employee` не перезаписывается, если уже установлен (для своевременного открытия/закрытия)
  2. **Синхронизация логики Celery с дашбордом:**
     - Используется `today_local` (дата в default_timezone) для всех объектов, затем локализуется в timezone объекта через `obj_tz.localize()`
     - Добавлена проверка `last_shift.status == 'completed' and not active_shifts_on_object` (как в дашборде)
     - Добавлена проверка "10 минут после закрытия": если прошло больше 10 минут, уведомление о закрытии не создается
     - Используется `localize()` вместо `replace(tzinfo=...)` для корректной работы с DST
  3. **Исправление установки статуса "Нет смен на объекте":**
     - При отсутствии смен добавлена проверка, что текущее время находится в рабочем времени объекта (`now_local >= expected_open and now_local <= expected_close`)
     - При установке статуса после закрытия добавлена проверка, что текущее время не превышает `expected_close + 2 часа` (чтобы статус не устанавливался глубокой ночью или на следующий день)
     - Используется timezone объекта для всех проверок времени
- **Результаты:**
  - ✅ При своевременном открытии/закрытии сотрудник корректно отображается на дашборде
  - ✅ Уведомления о раннем закрытии создаются только при реальном раннем закрытии (>5 минут)
  - ✅ Логика расчета в Celery полностью соответствует логике дашборда
  - ✅ Статус "Нет смен на объекте" устанавливается только в рабочее время объекта или вскоре после закрытия (не более 2 часов)
- **Файлы:**
  - `apps/web/routes/owner.py` — исправлена логика определения `opener_shift`, предотвращена перезапись `work_employee` при закрытии, добавлена проверка рабочего времени для статуса "Нет смен на объекте" (~60 строк изменений)
  - `core/celery/tasks/reminder_tasks.py` — синхронизирована логика расчета раннего закрытия с дашбордом (~80 строк изменений)
- **Правила документации:**
  - При изменении логики расчета времени закрытия/открытия объекта ВСЕГДА синхронизировать логику между дашбордом и Celery-задачами
  - Использовать `today_local` (дата в default_timezone) для всех объектов, затем локализовать через `obj_tz.localize()` в timezone объекта
  - При работе с `work_employee` не перезаписывать его при закрытии, если он уже установлен при открытии
  - При работе с временными зонами использовать `localize()` вместо `replace(tzinfo=...)` для корректной работы с DST
  - **ВСЕГДА проверять рабочее время объекта (`opening_time` и `closing_time`) при установке статуса "Нет смен на объекте"**
  - Статус "Нет смен на объекте" должен устанавливаться только если: время находится между `opening_time` и `closing_time` (для случая без смен) или не более чем через 2 часа после `closing_time` (для случая после закрытия всех смен)

### Улучшения страницы инцидентов: уведомления, отмена, фильтры, сортировка, пагинация, экспорт ✅

**Статус:** Завершено (10.01.2026)
- **Уведомления для инцидентов:**
  - Добавлены типы: `INCIDENT_CREATED`, `INCIDENT_RESOLVED`, `INCIDENT_REJECTED`, `INCIDENT_CANCELLED`
  - Шаблоны (plain, HTML, Telegram) для всех типов уведомлений
  - Отправка владельцу, сотруднику, управляющим объекта при создании/изменении статуса/отмене
  - Группа настроек "Инциденты" на `/owner/notifications`
  - Миграция: `a1b2c3d4e5f6_add_incident_notification_types.py` для добавления типов в enum и мета-таблицу
- **Действие отмены инцидента:**
  - Новый роут `POST /owner/incidents/{id}/cancel` с причинами отмены (дубль, не подтвердился, решение владельца)
  - Кнопка отмены в столбце "Действия" списка и на странице редактирования
  - Сохранение в истории через `IncidentHistory`, статус `cancelled`
  - Модальное окно для выбора причины отмены
- **Пагинация:**
  - Параметры `page` и `per_page` (25/50/100), сохранение фильтров в URL
  - Отображение "Показано X из Y", элементы управления пагинацией
- **Сортировка:**
  - Клик по заголовкам столбцов для сортировки (asc/desc), индикация активной сортировки
  - Параметры `sort_by` и `sort_order`, сохранение в URL
- **Множественные фильтры:**
  - Фильтр по статусам: множественный выбор через `statuses=new,in_review` (через запятую)
  - Фильтры по категории, объекту, сотруднику работают совместно
  - Подсветка активных фильтров, кнопка "Очистить фильтры"
- **Кнопка "Вернуться к списку":**
  - На страницах просмотра/редактирования с сохранением фильтров, сортировки, номера страницы
  - Параметр `return_url` передается через формы и ссылки
- **Экспорт:**
  - Роуты `GET /owner/incidents/export/excel` и `GET /owner/incidents/export/pdf`
  - Кнопка "Экспорт" с выпадающим меню (Excel/PDF)
  - Учитываются все фильтры и сортировка при экспорте
  - Excel: стилизованные заголовки, автоширина колонок, имя файла `incidents_YYYYMMDD_HHMMSS.xlsx`
  - PDF: таблица с информацией о фильтрах, поддержка кириллицы, имя файла `incidents_YYYYMMDD_HHMMSS.pdf`
- **Исправления:**
  - Добавлены переменные контекста `current_user`, `available_interfaces`, `new_applications_count` во все роуты инцидентов
  - Исправлена ошибка `ModuleNotFoundError: No module named 'shared.services.user_service'` - заменены импорты на локальную функцию `_get_db_user_id`
  - Исправлена ошибка Internal Server Error при отмене инцидента
- **Результаты:**
  - ✅ Уведомления для инцидентов полностью интегрированы
  - ✅ Действие отмены работает корректно с сохранением в истории
  - ✅ Пагинация, сортировка и фильтры работают совместно с сохранением состояния
  - ✅ Экспорт в Excel и PDF учитывает все фильтры и сортировку
  - ✅ Все элементы топбара (колокольчик, переключатель режима, профиль) отображаются на страницах инцидентов
- **Файлы:**
  - `domain/entities/notification.py` — добавлены типы уведомлений об инцидентах (~4 строки)
  - `shared/templates/notifications/base_templates.py` — добавлены шаблоны для инцидентов (~80 строк)
  - `migrations/versions/a1b2c3d4e5f6_add_incident_notification_types.py` — миграция для новых типов (~60 строк)
  - `shared/services/incident_service.py` — добавлены методы уведомлений и отмены (~200 строк)
  - `apps/web/routes/owner_incidents.py` — обновлены роуты (фильтры, сортировка, пагинация, экспорт, отмена, контекст) (~400 строк изменений)
  - `apps/web/templates/owner/incidents/list.html` — обновлен UI (фильтры, сортировка, пагинация, экспорт, модалка отмены) (~200 строк изменений)
  - `apps/web/templates/owner/incidents/edit.html` — добавлена кнопка возврата и отмены (~30 строк)
  - `apps/web/routes/owner.py` — добавлена категория "Инциденты" в настройки уведомлений (~1 строка)
- **Правила документации:**
  - При добавлении новых типов уведомлений об инцидентах обязательно добавлять в enum, шаблоны, мета-таблицу и обновлять `doc/vision_v1/shared/notifications.md`
  - При изменении роутов инцидентов обновлять `doc/vision_v1/roles/owner.md` с полным описанием параметров фильтрации, сортировки, пагинации
  - При добавлении новых действий (отмена, экспорт) обязательно документировать в `doc/vision_v1/roles/owner.md`
  - ВСЕГДА передавать `current_user`, `available_interfaces`, `new_applications_count` в контекст всех owner шаблонов для отображения топбара
  - При работе с инцидентами использовать локальную функцию `_get_db_user_id()` вместо несуществующего `get_user_id_from_current_user` из `shared.services.user_service`

## Недавние изменения (main, 09.01.2026)

### Исправление работы бота и уведомлений об объектах ✅

**Статус:** Завершено (09.01.2026)
- **Проблемы:**
  1. Бот не откликался на команду `/start`
  2. Уведомление о завершении смены не отправлялось
  3. Уведомление "нет активных смен" отправлялось во внерабочее время
  4. Ошибка `DatatypeMismatchError: column "type" is of type notificationtype but expression is of type character varying` при создании уведомлений
- **Причины:**
  1. Ошибка `NotificationService() takes no arguments` в `reminder_tasks.py` - был создан неиспользуемый экземпляр `NotificationService()` без необходимости
  2. Уведомление "нет активных смен" отправлялось без проверки рабочего времени объекта (отправлялось после `opening_time`, но не проверялось, что текущее время находится между `opening_time` и `closing_time`)
  3. В `_create_object_notification` использовался raw SQL INSERT с `CAST(:type AS notificationtype)`, но модель `Notification` использует `String(50)` для поля `type`, а не enum. При raw SQL возникал конфликт типов
- **Решение:**
  1. Удален неиспользуемый код создания `NotificationService()` в `reminder_tasks.py` в функции `_check_shifts_did_not_start_async`
  2. Исправлена логика отправки уведомлений "нет активных смен":
     - Добавлена проверка наличия `closing_time` у объекта
     - Добавлена проверка, что текущее время находится между `opening_time` и `closing_time` в локальном времени объекта
     - Используется `timezone_helper.utc_to_local(now)` для конвертации времени и сравнения с локальным временем объекта
  3. Заменен raw SQL INSERT на ORM (как в `notification_service.py`):
     - Используется `Notification()` объект с enum объектами
     - SQLAlchemy автоматически конвертирует enum объекты в строковые значения (`.value`)
     - Это устраняет конфликт типов между raw SQL и моделью
- **Результаты:**
  - ✅ Ошибка `NotificationService() takes no arguments` устранена
  - ✅ Ошибка `DatatypeMismatchError` при создании уведомлений устранена
  - ✅ Уведомления "нет активных смен" отправляются только в рабочее время объекта
  - ✅ Бот работает корректно после перезапуска
- **Файлы:**
  - `core/celery/tasks/reminder_tasks.py` — удален неиспользуемый код создания `NotificationService()`, исправлена логика проверки рабочего времени, заменен raw SQL на ORM для создания уведомлений (~70 строк изменений)
- **Правила документации:**
  - При работе с уведомлениями об объектах всегда проверять рабочее время объекта (`opening_time` и `closing_time`)
  - Использовать `timezone_helper.utc_to_local(now)` для конвертации времени в локальное время объекта перед сравнением
  - Не создавать неиспользуемые экземпляры сервисов, особенно в Celery задачах
  - **ВСЕГДА использовать ORM (`Notification()` объект) вместо raw SQL для создания уведомлений**, так как модель `Notification` использует `String` поля, а не enum, и SQLAlchemy автоматически конвертирует enum объекты в строковые значения
  - При создании уведомлений через ORM использовать `.value` для enum полей (SQLAlchemy сделает это автоматически, но лучше явно указывать)

### Улучшения страницы начислений: фильтры по дате выплаты и структура таблиц ✅

**Статус:** Завершено (09.01.2026)
- **Изменения:**
  1. **Фильтры:**
     - Заменены фильтры "Период с" и "Период по" на единый фильтр "Дата выплаты" на обеих вкладках
     - Добавлен фильтр "Сотрудник" рядом с фильтром "Объект" (включая уволенных сотрудников)
     - Галочка "Показать уволенных" теперь снимаемая (не всегда включена) и добавлена на обе вкладки
     - Фильтры применяются автоматически при изменении (убрана кнопка "Применить")
     - Состояние фильтров сохраняется при переключении вкладок и возврате из модальных окон
  2. **Вкладка "Сводка по сотрудникам":**
     - Добавлен столбец "Ставка" между "Начислений" и "Сумма"
     - Отображает название договора и ставку с символом рубля
     - Данные берутся из `entry_obj.contract` и `entry_obj.hourly_rate`
  3. **Вкладка "Начисления":**
     - Убраны столбцы "Часов" и "Ставка"
     - Столбец "Начислено" переименован в "За смены"
     - После столбца "К выплате" добавлены столбцы:
       - "Начислено" — сумма всех добавленных выплат (`payments_added`)
       - "Выплачено" — сумма подтвержденных выплат (`payments_completed`)
  4. **Сортировка и пагинация:**
     - Сортировка работает на всех записях (до пагинации), а не только на текущей странице
     - Все фильтры сохраняются в ссылках пагинации (включая `show_inactive`, `object_id`, `payment_date`, `employee_id`, `sort`, `order`)
- **Результаты:**
  - ✅ Фильтрация по дате выплаты вместо периода
  - ✅ Фильтр по сотруднику с динамической загрузкой при выборе объекта
  - ✅ Улучшенная структура таблиц с информацией о выплатах
  - ✅ Автоматическое применение фильтров и сохранение состояния
- **Файлы:**
  - `apps/web/routes/payroll.py` — обновлена логика фильтрации по дате выплаты, добавлен расчет сумм выплат, исправлена пагинация (~100 строк изменений)
  - `apps/web/templates/owner/payroll/list.html` — обновлены фильтры, структура таблиц, ссылки пагинации (~150 строк изменений)
  - `apps/web/routes/owner.py` — обновлен эндпоинт `api_employees_for_object` для включения уволенных сотрудников (~20 строк изменений)
  - `apps/web/services/payroll_service.py` — добавлен `selectinload` для загрузки `contract` и `employee` (~5 строк изменений)
- **Правила документации:**
  - При изменении фильтров на странице начислений учитывать, что фильтрация происходит по дате выплаты (вычисляется через `calculate_payment_date_for_entry`)
  - Фильтр "Сотрудник" загружается динамически при выборе объекта через `/owner/api/employees/for-object/{object_id}`
  - Столбец "Ставка" в сводке отображает название договора и ставку из активного начисления
  - Столбцы "Начислено" и "Выплачено" в таблице начислений показывают суммы всех выплат (добавленных и подтвержденных соответственно)
  - Все фильтры должны сохраняться в ссылках пагинации для корректной навигации

## Недавние изменения (main, 09.01.2026)

### Добавление уведомления "Смена не состоялась" ✅

**Статус:** Завершено (09.01.2026)
- **Задача:** Добавить новый тип уведомления для сценария, когда запланированная смена не состоялась (сотрудник не приступил к работе)
- **Логика шильдика "Не состоялась" в календаре:**
  - Шильдик выставляется в `calendar_filter_service.py` (строка 703-705), когда:
    - `end_local < now_local` — время окончания запланированной смены прошло
    - `not overlaps_actual` — нет пересечений с реальными сменами (смена не началась)
- **Решение:**
  1. **Новый тип уведомления:**
     - Добавлен `SHIFT_DID_NOT_START = "shift_did_not_start"` в `NotificationType` enum
     - Созданы шаблоны (plain, HTML, Telegram) в `base_templates.py` с описанием смены и сотрудника
     - Добавлен в мета-таблицу `notification_types_meta` через миграцию с описанием: "Смена была запланирована сотрудником, но он не приступил к работе"
  2. **Celery задача:**
     - Создана задача `check_shifts_did_not_start`, запускается каждые 10 минут
     - Проверяет смены, которые должны были начаться более часа назад (но не более 24 часов назад)
     - Отправляет уведомление владельцу и управляющим, если смена не началась (нет реальной смены со статусом `active` или `completed`)
     - Предотвращает дубликаты через проверку существующих уведомлений по `shift_schedule_id`
  3. **Сервис уведомлений:**
     - Добавлен метод `notify_shift_did_not_start()` в `ShiftNotificationService`
     - Отправляет уведомления владельцу объекта и управляющим с приоритетом `HIGH`
     - Использует шаблоны для рендеринга сообщений
- **Результаты:**
  - ✅ Новый тип уведомления добавлен в систему
  - ✅ Уведомление отправляется через час после планового начала смены (если смена не началась)
  - ✅ Шильдик в календаре появляется после окончания запланированной смены (если смена не состоялась)
  - ✅ Тип уведомления доступен в настройках владельца (`/owner/notifications`) в разделе "Смены"
- **Файлы:**
  - `domain/entities/notification.py` — добавлен `SHIFT_DID_NOT_START` в enum (~1 строка)
  - `shared/templates/notifications/base_templates.py` — добавлены шаблоны для нового типа уведомления (~10 строк)
  - `migrations/versions/f244dfa67bd3_add_shift_did_not_start_notification_.py` — миграция для добавления типа в мета-таблицу (~20 строк)
  - `core/celery/tasks/reminder_tasks.py` — добавлена задача `check_shifts_did_not_start` (~80 строк)
  - `core/celery/celery_app.py` — добавлена задача в `beat_schedule` (~5 строк)
  - `shared/services/shift_notification_service.py` — добавлен метод `notify_shift_did_not_start()` (~50 строк)
- **Правила документации:**
  - При добавлении новых типов уведомлений обязательно:
    - Добавлять в `NotificationType` enum
    - Создавать шаблоны в `base_templates.py`
    - Добавлять в мета-таблицу через миграцию с правильным описанием
    - Создавать Celery задачу для автоматической отправки (если требуется)
  - Логика шильдика "Не состоялась" в календаре: выставляется после окончания запланированной смены, если нет реальной смены
  - Уведомление отправляется через час после планового начала смены (если смена не началась)
