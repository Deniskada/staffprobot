...
- [ ] Логировать каждую `telegram.error.Conflict` отдельным событием + метрика `telegram_conflict_total`.
...
- [ ] Обновить `doc/DEVOPS_COMMAND_CENTER.md` и `doc/vision_v1/bot.md`: где смотреть lock, как запускать dev‑бота, как очищать очередь, что делать при подвисании.
- [ ] Добавить чек‑лист «Перед запуском dev‑бота»: убедиться, что lock свободен, использовать dev‑токен, чистить очередь после остановки.
