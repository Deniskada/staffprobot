...
## Итерация 44: Bot Instance Isolation & Monitoring (новая)

**Причина:** 18–19.11.2025 бот на проде перестал отвечать из-за `409 Conflict` от Telegram (в логах `telegram.error.Conflict: terminated by other getUpdates request`). Выяснено, что dev окружение запускает бота на продакшен‑токене (прописан в dev `.env`), из-за чего второй экземпляр отнимает очередь и прод замолкает. Нужна системная защита от повторного запуска и раздельные токены.

### Цели
1. **Развести токены** — гарантировать, что dev/stage никогда не используют продакшен‑бота.
2. **Защита запуска** — единственный активный экземпляр на проде, dev не poll’ит Telegram при чужом токене.
3. **Мониторинг и алерты** — фиксировать Conflicts/дубликаты процессов и очередь обновлений.
4. **Процедуры** — безопасные инструкции по сбросу очереди и запуску dev‑бота.

### План работ

#### Фаза 1. Разделение токенов и конфигурации
- [ ] Вынести токен бота в `.env` c раздельными значениями: `TELEGRAM_BOT_TOKEN_PROD`, `TELEGRAM_BOT_TOKEN_DEV`.
- [ ] Обновить dev `.env` (и `docker-compose.dev.yml`) чтобы по умолчанию использовать dev‑токен. Документировать в `doc/DEV_SETUP.md`.
- [ ] В CI/CD добавить проверку: если ревью/ручной запуск пытается использовать prod‑токен вне прод окружения — билд падает с подсказкой, где взять dev токен.

#### Фаза 2. Защита от двойного запуска
- [ ] Реализовать lock‑механику на старте бота (например Redis key `bot_polling_lock`). Инстанс пробует создать lock: если уже занят → лог + exit.
- [ ] В lock сохранять PID/host/время запуска. Команда `bot status` должна показывать, кто держит polling.
- [ ] Добавить CLI/management команду `python bot_main.py release-lock`, чтобы при аварийном завершении можно было освободить lock.

#### Фаза 3. Мониторинг и алерты
- [ ] Логировать все `Conflict` ошибки отдельным структурированным сообщением и считать их метрику (Prometheus `telegram_conflict_total`).
- [ ] Создать alert в DevOps Command Center (и/или Telegram уведомление админам) при первом `Conflict` за последние 10 минут.
- [ ] Добавить cron‑проверку: если бот 5 минут не обрабатывал обновления (нет записей в `bot_polling_heartbeat`), отправлять предупреждение.

#### Фаза 4. Документация и runbook
- [ ] Обновить `doc/DEVOPS_COMMAND_CENTER.md` и `doc/vision_v1/bot.md` (или создать новый раздел) с описанием:
  - как проверять активный инстанс (`docker logs`, lock, метрика);
  - как правильно запускать dev‑бота (всегда с dev токеном, через `drop_pending_updates` перед остановкой);
  - сценарий «бот завис» — какие команды выполнить, в каком порядке.
- [ ] Добавить инструкцию для QA/Dev: перед запуском dev‑бота обязательно убедиться, что lock свободен или используется dev токен.

#### Фаза 5. Тестирование и валидация
- [ ] На dev окружении: попытка запустить второй инстанс должна завершиться с понятной ошибкой и не затронуть прод polling.
- [ ] На stage/prod: проверить lock вручную, убедиться, что после рестарта бот автоматически освобождает lock и поднимается.
- [ ] Смок‑тесты: отправка команд ботам из разных окружений, верификация, что конфликты больше не появляются.

### DoD
- Конфликты больше не возникают при запуске dev бота.
- В мониторинге виден ID активного бота, есть alerting.
- Процедура устранения подвисания задокументирована и доступна DevOps/поддержке.
---
