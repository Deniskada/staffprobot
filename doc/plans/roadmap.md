...
<<<<<<< HEAD
**Общий прогресс:** 258/300 (86.0%)
=======
**Общий прогресс:** 254/258 (98.4%)
>>>>>>> parent of 94982da (docs: iteration 23 final report and roadmap update (100% complete))
...
- [ ] **3.1. Модель настроек уведомлений (1 день)**
  - Type: feature | Files: domain/entities/notification_preferences.py, migrations/versions/*
  - Acceptance: модель NotificationPreferences с полями user_id, preferences (JSON), email_verified, phone_verified, push_subscriptions

- [ ] **3.2. UI страница настроек (1 день)**
  - Type: feature | Files: apps/web/templates/shared/notification_settings.html, apps/web/routes/notifications.py
  - Acceptance: страница /settings/notifications; таблица с типами и каналами; API POST /api/notifications/preferences

### **Фаза 4: Расписание и группировка (2 дня)**
- [ ] **4.1. Планировщик уведомлений (1 день)**
  - Type: feature | Files: core/celery/tasks/notification_scheduler.py
  - Acceptance: Celery задача process_scheduled_notifications (каждые 5 мин); retry логика; обновление статусов

- [ ] **4.2. Группировка и дайджесты (1 день)**
  - Type: feature | Files: shared/services/notification_service.py
  - Acceptance: метод create_digest; Celery задачи send_hourly_digests и send_daily_digests; HTML дайджест

### **Фаза 5: Аналитика и отчеты (2 дня)**
- [ ] **5.1. Статистика уведомлений (1 день)**
  - Type: feature | Files: shared/services/notification_analytics.py, apps/web/routes/admin.py
  - Acceptance: метод get_notification_stats; метрики delivery/read/error rate; экспорт в JSON/CSV

- [ ] **5.2. UI страница аналитики (1 день)**
  - Type: feature | Files: apps/web/templates/admin/notification_analytics.html
  - Acceptance: страница /admin/notifications/analytics; графики; таблица ошибок; фильтры; экспорт

### **Фаза 6: Дополнительные функции (опционально, 2 дня)**
- [ ] **6.1. A/B тестирование шаблонов (опционально, 1 день)**
  - Type: feature | Files: shared/services/ab_testing_service.py
  - Acceptance: модель NotificationABTest; разделение на группы; трекинг метрик; определение победителя

- [ ] **6.2. Интеграции Slack/Discord (опционально, 1 день)**
  - Type: feature | Files: core/notifications/slack_channel.py, core/notifications/discord_channel.py
  - Acceptance: webhook интеграции; форматирование; настройки в системе

- [ ] **6.3. Система отписки (0.5 дня)**
  - Type: feature | Files: apps/web/routes/notifications.py
  - Acceptance: endpoint GET /unsubscribe/{token}; генерация токена; страница подтверждения

- [ ] **6.4. Защита от спама (0.5 дня)**
  - Type: feature | Files: core/utils/notification_rate_limiter.py
  - Acceptance: rate limiting max 10/час; исключения для urgent; логирование; уведомление админа

### **Фаза 7: Тестирование и документация (2 дня)**
- [ ] **7.1. Unit тесты (1 день)**
  - Type: test | Files: tests/unit/test_notification_service.py, tests/unit/test_channels.py
  - Acceptance: тесты NotificationService, каналов, шаблонов, планировщика, группировки; покрытие 90%+

- [ ] **7.2. Integration тесты (0.5 дня)**
  - Type: test | Files: tests/integration/test_notification_flow.py
  - Acceptance: тесты полного цикла, дайджестов, настроек, ошибок и ретраев

- [ ] **7.3. Документация (0.5 дня)**
  - Type: doc | Files: doc/vision_v1/shared/notifications.md, doc/plans/roadmap.md
  - Acceptance: описание системы, API endpoints, конфигурация каналов, примеры; roadmap обновлен
...
## 🎯 Итерация 23: UI/UX улучшения и исправления (октябрь 2025)
- [ ] **1. Добавить ссылку на бота на странице входа (1 час)**
  - [ ] 1.1. Добавить кнопку "Открыть бота в Telegram" на странице входа
    - Type: feature | Files: apps/web/templates/auth/login.html
    - Acceptance: кнопка открывает t.me/StaffProBot, на мобильных запускает приложение Telegram
- [ ] **2. Уточнить текст "Макс. расстояние" (30 минут)**
  - [ ] 2.1. Заменить "Макс. расстояние" на "Макс. расстояние от объекта для геолокации"
    - Type: improvement | Files: apps/web/templates/owner/{objects,timeslots}/*.html, apps/web/templates/manager/{objects,timeslots}/*.html
    - Acceptance: во всех формах и карточках текст уточнен, единообразие между owner/manager интерфейсами
- [ ] **3. Исправить баг с двойным @ в username (45 минут)**
  - [ ] 3.1. Добавить проверку username.startswith('@') во всех шаблонах
    - Type: bug | Files: apps/web/templates/{owner,manager,admin}/**/*.html
    - Acceptance: символ @ не дублируется, если username уже начинается с @
- [ ] **4. Изменить placeholder в полях "Почасовая ставка" (30 минут)**
  - [ ] 4.1. Заменить placeholder "500" на "Введите сумму"
    - Type: improvement | Files: apps/web/templates/owner/{timeslots,templates,objects,employees}/*.html
    - Acceptance: все поля почасовой ставки имеют нейтральный placeholder
