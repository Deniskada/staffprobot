...
**Статус:** В работе (Фаза 1: 14/14 задач ✅, Фаза 2: 2/2 задач ✅, Фаза 3: 0/21 задач)  
**Дата:** 5 ноября 2025  
**Приоритет:** Высокий  
...
- [ ] **Фаза 3: Управляющий**
  - [ ] 3.1. Дашборд управляющего (`/manager/dashboard`):
    - [ ] 3.1.1. "Доступные объекты" переименовать в "Объекты" и сделать кликабельными, переход по клику на `/manager/objects`
    - [ ] 3.1.2. "Активные смены" сделать кликабельными, переход по клику на `/manager/shifts?status=active&object_id=&dateRange=&date_from=&date_to=`
    - [ ] 3.1.3. "Сотрудники" сделать кликабельными, переход по клику на `/manager/employees`
    - [ ] 3.1.4. "Запланированные смены" переименовать в "Запланированные", показывать счетчик количества запланированных смен на сегодня (сейчас), по клику переход в `/manager/calendar` с автоматическим скроллом на сегодняшний день
    - [ ] 3.1.5. Удалить секцию "Быстрые действия"
