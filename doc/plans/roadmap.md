...
- [x] **1.6. Фильтр по подразделениям и унификация стилей (1 день)**
  ...
  - **Детали реализации:**
    ...
    - ✅ Загрузка подразделений через `OrgStructureService.get_org_tree()`
    - ✅ Фильтрация объектов по `org_unit_id` на стороне сервера
    - ✅ Dropdown фильтр подразделений с иерархическими отступами (`level * 12px`)
    - ✅ JavaScript каскадная фильтрация: выбор подразделения скрывает несоответствующие объекты
    - ✅ Функция `filterByOrgUnit()` - фильтрация + обновление URL
    - ✅ Функция `filterByObject()` - обновление URL
