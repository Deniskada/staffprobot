...
- [ ] **Фаза 5: Бот**
  - [ ] 5.1. Проанализировать текущий сценарий планирования смен через бота (`schedule_handlers.py`, `ScheduleServiceAdapter`, `TimeSlotService`) и зафиксировать точки интеграции
  - [ ] 5.2. Расширить `ScheduleService.get_available_time_slots_for_date` для возврата свободных интервалов по позициям (аналог веб-планировщика), сохранив совместимость старого ответа
  - [ ] 5.3. Обновить `ScheduleService.create_scheduled_shift_from_timeslot`: валидация пересечений по времени, сохранение выбранного интервала, поддержка нескольких сотрудников в слоте
  - [ ] 5.4. Переработать выбор слота в боте: показывать свободные интервалы (целиком), минимально изменив `handle_schedule_date_selection` и `handle_schedule_confirmation`
