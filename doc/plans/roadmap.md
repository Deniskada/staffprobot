...
- [ ] **1.1. Миграция БД: добавить use_contract_rate в contracts (0.5 дня)**
  - Type: migration | Files: migrations/versions/*.py, domain/entities/contract.py
  - Acceptance: 
    - Поле `use_contract_rate` (Boolean, default False) добавлено в таблицу `contracts`
    - Миграция применена на dev
    - Тесты миграции пройдены

- [ ] **1.2. Обновить модель Contract (0.5 дня)**
  - Type: feature | Files: domain/entities/contract.py
  - Acceptance:
    - Добавлено поле `use_contract_rate` в модель
    - Добавлен метод `get_effective_hourly_rate()` для определения применяемой ставки
    - Type hints и docstrings обновлены

- [ ] **1.3. Обновить ContractService (1 день)**
  - Type: feature | Files: apps/web/services/contract_service.py
  - Acceptance:
    - Метод `create_contract()` учитывает `use_contract_rate`
    - Метод `update_contract()` обрабатывает изменение флага
    - Добавлена валидация: если `use_contract_rate=True`, `hourly_rate` обязателен
    - Тесты покрытие ≥80%

- [ ] **1.4. Обновить логику открытия смены (1 день)**
  - Type: feature | Files: shared/services/shift_service.py, apps/bot/services/shift_service.py
  - Acceptance:
    - При открытии смены проверяется флаг `use_contract_rate` активного договора
    - Приоритет: contract_rate (если use_contract_rate=True) → timeslot_rate → object_rate
    - Логирование источника ставки
    - Тесты покрытие ≥80%

- [ ] **1.5. Обновить UI договоров (1 день)**
  - Type: feature | Files: apps/web/templates/owner/employees/create.html, apps/web/templates/owner/employees/edit_contract.html
  - Acceptance:
    - Добавлен чекбокс "Использовать ставку из договора" с пояснением
    - При включении чекбокса поле "Почасовая ставка" становится обязательным
    - Отображается источник ставки на странице детализации договора
    - Мобильная версия корректна

- [ ] **1.6. Тестирование Задачи 1 (0.5 дня)**
  - Type: test | Files: tests/unit/test_contract_rate.py, tests/integration/test_shift_rate_priority.py
  - Acceptance:
    - Unit-тесты для определения приоритета ставок
    - Integration-тесты: создание договора → открытие смены → проверка ставки
    - Покрытие ≥80%
...
- [ ] **3.1. Создать модель PaymentSchedule (1 день)**
  ...
  - Acceptance:
    - Таблица `payment_schedules` создана:
      ...
      - `payment_period` (за какой период: week, month, custom) - период за который производятся выплаты
