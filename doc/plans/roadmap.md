...
- [ ] На старте бота создавать Redis‑lock (`bot_polling_lock`). Если ключ занят — логируем владельца lock и завершаем процесс.
- [ ] В lock хранить host/PID/время. Добавить CLI команду `python bot_main.py release-lock` для аварийного снятия.
