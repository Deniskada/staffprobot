...
- [ ] **Фаза 3: Управляющий**
  ...
  - [ ] 3.18. Календарь управляющего: внедрить новую модель занятости, унифицировать клики (тайм-слот → модалка, смена → страница), обновить fallback API и UI  
        - [ ] Реализовать fallback расчёта свободного времени (shared service + manager API)  
        - [ ] Обновить клиентский JS: тайм-слоты, бейджи, модалки
  - [ ] 3.19. Планировщик смен управляющего: адаптировать подсказки и валидации под новую модель свободного времени, сохранить корректный `return_to`
