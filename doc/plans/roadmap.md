...
- [x] **Фаза 1: Владелец** ✅
  ...
  - [ ] 1.11. Календарь владельца: пересчитать занятость тайм-слотов по времени (учёт `max_employees`), ввести статусы «Свободно / Запланирована / Не состоялась», обновить API и UI  
        - [x] Обновить визуальные бейджи статусов (правый верхний угол, единый стиль)  
        - [ ] Довести расчёт `occupancy_ratio` и fallback API для всех ролей (owner/manager/employee)  
        - [ ] Финально протестировать сценарии «Свободно / Запланирована / Не состоялась», обновить документацию
  - [ ] 1.12. Планировщик смен владельца: адаптировать подсказки и проверки под новую модель свободного времени, обеспечить корректный возврат по `return_to`  
        - [ ] Обновить подсказки/валидации на новой странице планирования  
        - [ ] Синхронизировать поведение с правками в `UniversalCalendarManager`
...
- [ ] **Фаза 4: Сотрудник**
  ...
  - [ ] 4.2. Планирование смен сотрудника: страница по клику на смену/тайм-слот (аналог owner/manager) с автоопределением сотрудника, предзаполнением объекта и загрузкой свободных тайм-слотов
