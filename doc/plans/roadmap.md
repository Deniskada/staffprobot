...
**Статус:** В работе (Фаза 1: 14/14 задач ✅, Фаза 2: 0/2 задач, Фаза 3: 0/21 задач)  
**Дата:** 5 ноября 2025  
**Приоритет:** Высокий  
...
- [ ] **Фаза 2: Форма логина**
  - [ ] 2.1. `/auth/login` — исправить обработку PIN-кода: после нажатия "Получить код", ввода кода и нажатия "Войти в систему" код должен приниматься корректно (сейчас получается ошибка "Неверный PIN-код или время истекло", остается только автологон после ввода кода)
  - [ ] 2.2. В мобильной версии кнопка "Получить код" должна быть представлена только значком без текста "Получить код"
