...
- [ ] **3.1. Создать модель PaymentSchedule (1 день)**
  ...
  - Acceptance:
    ...
    - Seed-скрипт: "Еженедельно по пятницам", "Два раза в месяц (15-е и 30-е)", "Ежемесячно 5-го числа"
    - Миграция применена на dev
...
- [ ] **3.3. Создать модели Payroll (начисления/удержания/выплаты) (1.5 дня)**
  ...
  - Acceptance:
    ...
    - Таблица `payroll_deductions` - удержания:
      - `id`, `payroll_entry_id` (FK), `deduction_type`, `amount`, `description`
    - Таблица `employee_payments` - выплаты:
      - `id`, `payroll_entry_id` (FK), `payment_date`, `amount`, `payment_method`, `status`, `notes`
    - Миграции применены на dev

- [ ] **3.4. Создать PayrollService (2 дня)**
  - Type: feature | Files: apps/web/services/payroll_service.py
  - Acceptance:
    - Метод `calculate_payroll(employee_id, period_start, period_end)` - расчет за период
    - Метод `create_payroll_entry()` - создание начисления
    - Метод `add_deduction()` - добавление удержания
    - Метод `record_payment()` - запись выплаты
...
- [ ] **3.5. UI страницы выплат для Owner (2 дня)**
  ...
  - Acceptance:
    ...
    - Страница `/owner/payroll/{entry_id}` - детализация начисления:
      ...
      - Начисления, удержания, итоговая сумма
      - Кнопки: "Одобрить", "Записать выплату", "Отклонить"
...
- [ ] **5.3. Обновить документацию (1 день)**
  ...
  - Acceptance:
    - Обновлен `doc/vision_v1/entities/contract.md` - новые поля и логика
    - Создан `doc/vision_v1/entities/payment_system.md`
    - Создан `doc/vision_v1/entities/payroll.md`
    - Создан `doc/vision_v1/entities/org_structure.md`
    - Обновлен `doc/vision_v1/roles/owner.md` - новые страницы
    - Обновлен `doc/vision_v1/roles/employee.md` - страница выплат
    - Обновлен `doc/vision.md` - схемы таблиц
    - Соответствие `doc/DOCUMENTATION_RULES.md`

- [ ] **5.4. E2E тестирование полного сценария (1 день)**
  - Type: test | Files: tests/integration/test_payment_e2e.py
  - Acceptance:
    - Сценарий 1: Владелец создает подразделение → объект → договор с `use_contract_rate=True` → сотрудник открывает смену → проверка ставки
    ...
    - Сценарий 3: Владелец рассчитывает начисления за месяц → одобряет → записывает выплату → сотрудник видит историю
    - Все тесты проходят
...
**Общая оценка времени:** 4-5 недель (20-25 рабочих дней)
