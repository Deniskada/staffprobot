...
- [ ] **Фаза 3: Управляющий**
  ...
  - [ ] 3.15. Страница `/manager/profile`:
    - [ ] Убрать секцию "Доступные объекты" из профиля управляющего
    - [ ] Исправить счетчики в разделе "Статистика", чтобы показывать правильные цифры
  - [ ] 3.16. Проверка ограничений доступа для управляющего:
    - [ ] Проверить, что при генерации отчетов управляющему доступны только те объекты и сотрудники, которые разрешены ему по договору (не все объекты и сотрудники)
    - [ ] Проверить то же самое для страницы `/manager/payroll/adjustments` (корректировки начислений)
    - [ ] Проверить то же самое для страницы `/manager/payroll` (начисления)
  - [ ] 3.17. Страница `/manager/payroll`:
    - [ ] Реализовать фильтрацию, сортировку и пагинацию такие же как на `/owner/shifts/`
    - [ ] 3.17.1. В столбце "Последнее начисление" выводить временной период начисления с самым большим ID
    - [ ] 3.17.2. При нажатии на кнопку "Детали" открывать страницу всех начислений сотрудника:
      - [ ] Страница должна быть аналогична странице деталей начисления, но с таблицей начислений сверху
      - [ ] При клике на начисление в таблице должна меняться остальная часть страницы с данными выбранного начисления
      - [ ] Выплаты по начислению должны показываться внизу страницы перед протоколом изменений
      - [ ] Кнопки "Добавить" в разделах "Удержания" и "Премии" должны быть активны только для начислений, по которым не было выплат
      - [ ] Для доступных начислений кнопки активны и передают в модальные формы информацию об актуальном начислении
      - [ ] Убедиться, что в модальных формах такие же поля (категории, подкатегории и т.д.), как и у владельца
