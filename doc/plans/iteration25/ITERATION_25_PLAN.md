...
**Задачи:**
- [ ] Создать роутер `admin_notifications` с префиксом `/admin/notifications`
- [ ] Реализовать `admin_notifications_dashboard()` - главный дашборд
- [ ] Реализовать `admin_notifications_list()` - список с фильтрами
- [ ] Реализовать `admin_notifications_analytics()` - детальная аналитика
- [ ] Добавить проверку роли суперадмина через `require_superadmin`
- [ ] Интегрировать с `apps/web/app.py` в секцию админки (строка 19)
- [ ] Добавить базовые API endpoints для AJAX (`/admin/api/notifications/*`)
- [ ] Учесть существующий endpoint `/admin/api/notifications` в `billing.py`
...
**Задачи:**
- [ ] Создать `AdminNotificationService` как расширение существующего `NotificationService`
- [ ] Реализовать `get_notifications_stats()` - общая статистика (включая PaymentNotification)
...
- [ ] Реализовать `get_user_analytics()` - топ пользователей
- [ ] Интегрировать с существующим кэшированием из `NotificationService`
- [ ] Использовать существующие индексы из таблицы `notifications`
...
- [ ] Добавить таблицу топ пользователей по активности
- [ ] Добавить статус каналов доставки (работает/не работает)
- [ ] Добавить последние уведомления (10 записей)
- [ ] Интегрировать с админским layout (`admin/base_admin.html`)
- [ ] Добавить автоматическое обновление данных (каждые 5 мин)
