# План финализации: Rules Engine + Tasks v2 + Incidents
...
### 4.4 Автоматические корректировки
...
2. [ ] Дубли сообщений после фото-отчёта
3. [ ] "📝 Задачи на смену: —" в спонтанных сменах (должен показывать задачи)
4. [ ] Автозапуск планировщика при CRUD плана

### Фаза 2: Smoke-тесты основных потоков (2-3 дня)
1. [ ] **E2E-тест (запланированная смена)**:
   - Создать план (мульти-объекты, повтор)
   - Открыть смену по тайм-слоту
   - "Мои задачи" → отметить задачу с фото
   - Закрыть смену
   - Проверить: TaskEntryV2, PayrollAdjustment (shift_base, task_bonus, late_penalty)
