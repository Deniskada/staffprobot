...
async def owner_calendar(
    ...
    """Календарный вид планирования - редирект на недельный вид"""
    current_user = await get_current_user(request)
    if not current_user or current_user.get("role") != "owner":
        return RedirectResponse(url="/auth/login", status_code=status.HTTP_302_FOUND)
    
    # Редиректим на недельный вид с текущей неделей
    ...
    redirect_url = f"/owner/calendar/week?year={year}&week={week_number}"
    if object_id:
        redirect_url += f"&object_id={object_id}"
