...
from .notification_auto_login_vars import enrich_variables_with_action_link
from .messenger_account_service import get_max_external_user_id_for_user
from .senders.telegram_sender import get_telegram_sender
from .senders.max_sender import get_max_sender
...
class NotificationDispatcher:
    ...
    def __init__(self):
        ...
        self.max_sender = get_max_sender()
    ...
    async def dispatch_notification(
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                max_ext: Optional[str] = None
                if notification.channel_enum == NotificationChannel.MAX:
    ...
    async def _send_via_channel(
        ...
        notification: Notification,
        user: User,
        max_external_user_id: Optional[str] = None,
    ) -> bool:
        """
        ...
        try:
            if notification.channel_enum == NotificationChannel.TELEGRAM:
                ...
                variables = await enrich_variables_with_action_link(
                    notification, user, dict(notification.data or {})
                )
            ...
            elif notification.channel_enum == NotificationChannel.MAX:
                if not max_external_user_id:
                    logger.warning(
                        f"User {user.id} has no MAX binding (notification_id={notification.id})"
                    )
                    return False
                variables = await enrich_variables_with_action_link(
                    notification, user, dict(notification.data or {})
                )
                return await self.max_sender.send_notification(
                    notification=notification,
                    max_user_id=max_external_user_id,
                    variables=variables,
                )
